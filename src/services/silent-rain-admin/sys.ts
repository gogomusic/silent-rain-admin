...
export async function sysControllerChangePwdCode(
  ...
  return request<API.ResponseDto & { data?: any }>('/sys/changePwdCode', {
    method: 'GET',
    params: {
      ...params,
    },
...
export async function sysControllerRegister(
  ...
  return request<API.ResponseDto & { data?: any }>('/sys/register', {
    method: 'POST',
    headers: {
...
/** 获取注册验证码 注册验证码将发送至用户邮箱 GET /sys/registerCode */
export async function sysControllerRegisterCode(
  // 叠加生成的Param类型 (非body参数swagger默认没有生成对象)
  params: API.SysControllerRegisterCodeParams,
  options?: { [key: string]: any },
) {
  return request<API.ResponseDto & { data?: any }>('/sys/registerCode', {
    method: 'GET',
    params: {
      ...params,
    },
